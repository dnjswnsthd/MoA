        ...
        <v-row class="centerContent my-10 col-12 mx-4">
          ...
          >
            <div @click="goDetail(funding.project_num)">
              <img src="@/assets/images/funding/fox.jpg" class="fullWidth" alt="예시" />
              <br />
              <div>
                <v-progress-linear
                  color="#CE93D8"
  ...
  created() {
    ...
    http
      ...
      .then(({ data }) => {
        ...
        for (var i = 0; i < 6; i++) {
          let startdate = this.fundingDatas[i].start_date;
          let deadline = this.fundingDatas[i].deadline;
          ...
          let gap = Math.floor((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24));
          let gap2 = Math.floor(
